...
- Preserve opacity while replacing textures. Unsupported material and instance structures must be reported rather than silently changed.
...
- Reusable shared presets are immutable templates. Users edit session copies and can import or export JSON.
...
- Latest user direction: use test files and an isolated process for verification. Do not continue computer access to the occupied Composer session.
